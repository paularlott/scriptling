...
// runtime.http / MCP / WebSocket concurrency model. When served over HTTP the
// originating request is available to handlers via get_request() /
// request_context().
var JSONRPCSubLibrary = func() *object.Library {
...
Example:
  def divide(params):
	...
	// middleware's context dict from method handlers, which receive only
	// their params. Over stdio there is no request: get_request() returns
	// None and request_context() an empty dict.
