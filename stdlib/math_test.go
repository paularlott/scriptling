...
func TestMathLog(t *testing.T) {
	...
	// Test error case
	result = log.Fn(context.Background(), &object.Integer{Value: 0})
...
func TestMathFmod(t *testing.T) {
	...
	// Test error case
	result = fmod.Fn(context.Background(), &object.Float{Value: 5.0}, &object.Float{Value: 0.0})
	if _, ok := result.(*object.Error); !ok {
...
func TestMathFactorial(t *testing.T) {
	...
	// Test error cases
	result = factorial.Fn(context.Background(), &object.Integer{Value: -1})
	if _, ok := result.(*object.Error); !ok {
		t.Errorf("factorial(-1) should return error, got %T", result)
	}

	result = factorial.Fn(context.Background(), &object.Integer{Value: 21})
	if _, ok := result.(*object.Error); !ok {
