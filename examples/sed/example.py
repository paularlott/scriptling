...
# ── Literal replacement ────────────────────────────────────────────────────────
# sed.replace(old, new, path, ...) treats the search string exactly as written.
...
# sed.replace_pattern(regex, new, path, ...) uses Go regular expressions.
...
n = sed.replace_pattern(r"def get_(\w+)\(", "def fetch_${1}(", tmp2)
...
matches = grep.string("import scriptling.sed", demo_dir, recursive=True, glob="*.py")
files = {m["file"] for m in matches}
print(f"Files referencing scriptling.sed: {len(files)}")
...
n = sed.replace("todo:", "DONE:", tmp3, ignore_case=True)
print(f"case-insensitive replace: {n} file(s) modified")
print(f"Result:\n{os.read_file(tmp3)}")
os.remove(tmp3)

# ── Extract capture groups ────────────────────────────────────────────────────
# sed.extract(regex, path, ...) returns capture groups from every match.
...
matches = sed.extract(r"def (\w+)\(", tmp4)
...
matches = sed.extract(r"(\w+)=(\S+)", tmp5)
print(f"\nConfig entries:")
for m in matches:
    key, value = m["groups"]
    print(f"  {key} = {value}")
