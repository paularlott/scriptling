...
enc = urllib.parse.quote(special)
...
relative = "/users/123"
joined = urllib.parse.urljoin(base, relative)
print("Joined '" + base + "' + '" + relative + "' = '" + joined + "'")

# urlsplit
print("\n--- URL Splitting ---")
split_parts = urllib.parse.urlsplit("https://example.com/path?query=value#fragment")
...
qs_dict = urllib.parse.parse_qs(qs_str)
...
encoded1 = urllib.parse.urlencode(data1)
print("urlencode " + str(data1) + " = '" + encoded1 + "'")

data2 = {"tags": ["python", "web", "api"], "active": "true"}
encoded2 = urllib.parse.urlencode(data2)
