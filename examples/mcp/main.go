	...
	server.RegisterTool(
		...
		func(ctx context.Context, req *mcp.ToolRequest) (*mcp.ToolResponse, error) {
			...
			if infoType == "libraries" || infoType == "all" {
				...
				response.WriteString("import url\n")
				response.WriteString("encoded = url.quote('hello world')\n")
				response.WriteString("parts = url.urlsplit('https://example.com/path')\n")
				response.WriteString("query = url.urlencode({'key': 'value'})\n")
